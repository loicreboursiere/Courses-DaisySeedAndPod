//Chanson de la chorale des fantômes (ghost choir)
...
{
    //int freq      = (int)p_freq.Process(); //idée - choix modes : mélodie entière ou principale avec accords
    ...
    notes[0] = notes[0] + (12 * (inversion >= 1)); //note fonda inversée ou non
    notes[1] = notes[1] + (12 * (inversion >= 2)); //freq note fonda + freq pour atteindre pro notes de l'accord + 12 demi tons si inversion >=2 
    notes[2] = notes[2] + (12 * (inversion >= 3));
    notes[3] = notes[3] + (12 * (inversion >= 4)); //si inversion 4 est atteinte inverse tout l'odre des accords (augmente de 12 tout)
